...
tf.app.flags.DEFINE_integer('max_steps', 17000, 'the max training steps ')
...
tf.app.flags.DEFINE_string('mode', 'f', 'Running mode. One of {"train", "valid", "test"}')
     ...
     # batch data 获取
        ...
        try:
            ...
            while not coord.should_stop():
                ...
                _, loss_val, train_summary, step, acc = sess.run(
                    [graph['train_op'], graph['loss'], graph['merged_summary_op'], graph['global_step'],graph['accuracy']],
                    feed_dict=feed_dict)
                
                train_writer.add_summary(train_summary, step)
                end_time = time.time()
                logger.info("the step {0} takes {1} loss {2} accuracy {3}".format(step, end_time - start_time, loss_val, acc))
...
def inference2():
    ...
    with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options,allow_soft_placement=True)) as sess:
        ...
        for item in image_set:
            temp_image = item
            probabilities = sess.run([graph['p']],
                                    feed_dict={graph['images']: temp_image,
                                               graph['keep_prob']: 1.0,
                                                graph['is_training']: False})
            label_dict = get_label_dict()
            list1.append(dict_new[lang_char[label_dict[int(probabilities[0][0])]]])
            list2.append(label_dict[int(probabilities[0][0])])
        print(list1)
        print(list2)
