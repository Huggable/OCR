...
class PreprocessResizeKeepRatioFillBG(object):
    ...
    def put_img_into_center(cls, img_large, img_small, ):
        ...
        #print(start_height,height_small,start_width,width_small)
    ...
    def do(self, cv2_img):
        ...
        if not self.fill_bg:
            ...
            if pix_dim is not None:
                norm_img = np((height_minus_margin,
                                     width_minus_margin,
                                     pix_dim),
                                    np.uint8)
            else:
...
class Font2Image(object):
    ...
    def do(self, font_path, char, rotate=0):
        ...
        # 白色字体
        draw.text((0, 0), char, k,
                  font=font)
        if rotate != 0:

            im2 = img.convert('RGBA')
            img = im2.rotate(rotate)

            fff = Image.new('RGBA',img.size, 255)
...
python gen_printed_char.py --out_dir ./dataset \
    ...
    # 合并成新的映射关系表：（汉字：ID）
    klang_chars = dict(zip(char_list,value_list))



    for (char, value) in klang_chars.items():
        try:
            newchar = zh2Hant[char]
            char_list.append(newchar)
        except Exception:
            pass
        finally:
            pass
    ...
    for i in range(len(char_list)):
        value_list.append(i)
    nlang_chars = dict(zip(char_list, value_list))


    lang_chars = {}
    for i,char in enumerate(nlang_chars):
        lang_chars[char] = i
    print(len(lang_chars))
    f = open('./dictionary.txt', 'w')
    f.write(str(lang_chars))
    f.close()
    ...
    print('font can be used:    '+str(len(verified_font_paths)))
